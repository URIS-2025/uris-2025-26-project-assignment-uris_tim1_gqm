...
public class OrchestrationClient : IOrchestrationClient
{
    ...
    public async Task StartWorkflowAsync(Guid goalId)
    {
        try
        {
            var json = JsonSerializer.Serialize(new { goalId });
            var content = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await _httpClient.PostAsync("/workflow", content);
            if (!response.IsSuccessStatusCode)
                _logger.LogWarning("Failed to start workflow for GoalId {GoalId}. Status: {Status}", goalId, response.StatusCode);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "OrchestrationClient.StartWorkflowAsync failed for GoalId {GoalId}.", goalId);
        }
    }
